...
from metaforge._utils import file_line_generator
...
class AngParser(MetaForgeParser):
  ...
  def parse_hkl_family(self, tokens: List[str]) -> HKLFamily:
    h = int(tokens[0])
    k = int(tokens[1])
    l = int(tokens[2])
    s1 = int(tokens[3])
  ...
  def parse_families(self, file: Generator[str, None, None], num_families: int) -> List[HKLFamily]:
    families = []
    for _ in range(num_families):
      ...
      families.append(self.parse_hkl_family(tokens[2:]))
    return families

  def parse_categories(self, tokens: List[str]) -> List[int]:
    tokens[0] = tokens[0][len(ANG_CATEGORIES):]
    if not tokens[0]:
      tokens.pop(0)
    return [int(value) for value in tokens]

  def join_lines_until(self, file: Generator[str, None, None], sentinel: str) -> str:
    return '\n'.join([line for line in iter(lambda: next(file).rstrip('\n'), sentinel)])

  def parse_notes(self, file: Generator[str, None, None]) -> str:
    return self.join_lines_until(file, ANG_END_NOTES)
    ...
    for line in file_gen:
      if not line.startswith(ANG_HEADER_CHAR):
        ...
        if keyword == ANG_HEADER:
          ...
          ang_header.notes = self.parse_notes(file_gen)
        elif keyword == ANG_COLUMN_NOTES and tokens[1] == ANG_START:
          ang_header.col_notes = self.parse_column_notes(file_gen)
          ...
          current_phase.hkl_families = self.parse_families(file_gen, num_families)
        elif keyword.startswith(ANG_CATEGORIES):
          current_phase.categories = self.parse_categories(tokens)
